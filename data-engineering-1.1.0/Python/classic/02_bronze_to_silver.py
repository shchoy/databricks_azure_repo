...
# TODO
bronzeDF = spark.read.table("health_tracker_classic_bronze").where("status = 'new'")
...
bronzeAugmentedDF = bronzeDF.withColumn("nested_json", from_json("value", json_schema))
...
silver_health_tracker.head(10)
silver_health_tracker.show(10)
...
#silver_health_tracker.filter(col("name").like("%Gonzalo%")).show()
display(silver_health_tracker.filter(col("name").like("%Gonzalo%")))
...
display(silver_health_tracker_clean)
silver_health_tracker_clean.count()
...
bronzeTableDF = bronzeTable.toDF()
bronzeTableDF.show()
