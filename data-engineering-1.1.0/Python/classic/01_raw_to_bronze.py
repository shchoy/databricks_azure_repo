...
raw_health_tracker_data_df = (
  raw_health_tracker_data_df.select(
    lit("file.training.databricks.com").alias("datasource"),
...
raw_health_tracker_data_df.show()
raw_health_tracker_data_df.count()
...
raw_health_tracker_data_df.write.format("delta").mode("append").partitionBy("p_ingestdate").save(bronzePath)
...
# MAGIC SELECT count(*) FROM health_tracker_classic_bronze
